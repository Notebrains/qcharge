...
class _MonthPickerDialogState extends State<_MonthPickerDialog> {
        ...
        child: Dialog(
          ...
          Builder(builder: (context) {
            ...
            return IntrinsicHeight(
              child: Row(
                  mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [header, content]),
            );
          })
        ])));
  }
  ...
  Widget _getMonthButton(final DateTime date, final ThemeData theme, final String locale) {
    ...
    if (_firstDate.compareTo(date) <= 0 && _lastDate.compareTo(date) >= 0)
      callback = () => setState(() => selectedDate = DateTime(date.year, date.month));
    else if (_firstDate.compareTo(date) <= 0)
      callback = () => setState(() => selectedDate = DateTime(date.year, date.month));
    else if (_lastDate.compareTo(date) >= 0) callback = () => setState(() => selectedDate = DateTime(date.year, date.month));

    return FlatButton(
      onPressed: () {
        callback();
      },
      color: date.month == selectedDate.month && date.year == selectedDate.year ? theme.accentColor : null,
