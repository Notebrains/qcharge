...
class _StopState extends State<Stop> {
  ...
  Future<bool> getConnectorDetails()async{
    ...
    http.Response checkStatus = await http.get(Uri.parse("https://qcharge.hashtrix.in/public/api/getchargerstatus/${connectorData["stationId"]}/${connectorData["chargerId"]}/${connectorData["connector"]["connectorId"]}"));
...
//      elapsedTime = "${temp.substring(0,1).toUpperCase}${temp.substring(1)}";
      ...
      statusTimer = Timer.periodic(Duration(seconds: 3), (timer) async {
        ...
        if (statusData["data"].toString() == "charged") {
          ...
          data["connectorId"] =
              connectorData["connector"]["connectorId"].toString();
          data["cardNo"] = "ABCDE";
          ...
          try {
            http.Response response = await http.post(
                Uri.parse("${Constants.APP_BASE_URL}stopcharging"), body: data);
                          ...
                          Padding(
                            ...
                            child: Button(
                              ...
                              onPressed: () async {
                                ...
                                Map<String, dynamic> data = Map();
                                data["chargerId"] =
                                    connectorData["chargerId"].toString();
                                data["connectorId"] =
                                    connectorData["connector"]["connectorId"]
                                        .toString();
                                data["cardNo"] = "ABCDE";

                                String? token = await MySharedPreferences()
                                    .getApiToken();
                                data["token"] = token.toString();
                                try {
                                  http.Response response = await http.post(
                                      Uri.parse("${Constants
                                          .APP_BASE_URL}stopcharging"),
                                      body: data);
                                  print("stopCharge: ${response.statusCode}");
                                  print("stopCharge: ${response.body}");
                                  setState(() {
...
//                    widget.onTap();
                ...
                return Center(child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text(" Connector not found", style: TextStyle(color: AppColor.app_txt_white,
                      fontSize: 12,
                      fontWeight: FontWeight.normal,),),
                    Padding(
                      padding: const EdgeInsets.only(left: 36,
